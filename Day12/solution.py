...
def calculate_part_2(test=False):
    ...
    return sum(count_ways(s, l) for s, l in scaled)


def parse_line(line):
    part1, part2 = line.split()
    return part1, tuple(int(i) for i in part2.split(","))
...
def count_ways(symbols, lengths):
    ...
    for before_symbols, after_symbols in options:
        sub_result = 1
        if num_before > 0:
            sub_result *= count_ways(before_symbols, tuple(lengths[:index]))
        if num_after > 0:
            sub_result *= count_ways(after_symbols, tuple(lengths[index + 1 :]))
        result += sub_result
    return result
